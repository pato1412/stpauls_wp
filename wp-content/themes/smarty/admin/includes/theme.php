...
add_action('wp_ajax_smarty_install_plugin', 'smarty_install_plugin');
function smarty_install_plugin()
{
    check_ajax_referer('smarty_install_plugin', 'security');
    $r = array();
    $plugins = smarty_register_required_plugins(true);
    $layout = sanitize_text_field($_GET['layout']);
    $layout_plugins_config = smarty_layout_plugins($layout);
    ...
    if (empty($plugin_slug) and !empty($plugins[$plugin_slug])) {
        wp_send_json(array('error' => esc_html__('Error occured', 'smarty')));
        exit;
    }
    ...
    $next = smarty_get_next($plugins, $plugin_slug);
    ...
    if(!empty($source)) {
        $installed = (smarty_check_plugin_active($plugin_slug)) ? true : $plugin_upgrader->install($source);
        if(is_wp_error($installed)) {
            ...
            smarty_activate_plugin($plugin_slug);
            $r['installed'] = true;
            $r['activated'] = true;
...
function smarty_activate_plugin($slug) {
    activate_plugin(smarty_get_plugin_main_path($slug));
}
...
function smarty_get_next($array, $key)
{
