			...
			vc_add_params( 'vc_tta_tabs', array(
				array(
					...
					'value' => array(
						...
						esc_html__( 'Align right', 'smarty' )  => 'style-3',
					)
				)
			));
...
if ( function_exists( 'vc_map' ) ) {
	...
	if ( ! function_exists('smarty_vc_map_shortcodes') ) {
		...
		function smarty_vc_map_shortcodes() {
			...
			vc_map( array(
				...
				'params'      => array(
					...
					array(
                        ...
                        'dependency' => array(
                            'element' => 'view',
                            'value' => array('carousel', 'list', 'grid')
                        )
					),
					array(
						'type'       => 'checkbox',
            ...
            // Teacher Department
            $teacher_department = get_terms( 'stm_course_department', array(
                'hide_empty' => false,
            ) );
            $teacher_department_list = array(
                esc_html__('Select', 'smarty') => 0
            );

            if(!is_wp_error($teacher_department) and !empty( $teacher_department ) ) {
                foreach( $teacher_department as $teacher_category ) {
                    $teacher_department_list[$teacher_category->name] = $teacher_category->slug;
                }
            }
			...
			vc_map( array(
				...
				'params'      => array(
                    array(
                        'type' => 'dropdown',
                        'heading' => esc_html__( 'Sort By', 'smarty' ),
                        'param_name' => 'teachers_sort_by',
                        'value' => array(
                            esc_html__( 'Categories', 'smarty' ) => 'categories',
                            esc_html__( 'Department', 'smarty' ) => 'department'
                        )
                    ),
                    array(
                        'type'       => 'dropdown',
                        ...
                        'dependency' => array(
                            'element' => 'teachers_sort_by',
                            'value' => 'categories'
                        ),
                    ),
                    array(
                        'type'       => 'dropdown',
                        'heading'    => esc_html__( 'Department', 'smarty' ),
                        'param_name' => 'teachers_department',
                        'value' => $teacher_department_list,
                        'dependency' => array(
                            'element' => 'teachers_sort_by',
                            'value' => 'department'
                        ),
			...
			// Courses
            $course_categories = get_terms( 'stm_course_department', array(
                'hide_empty' => false,
            ) );
            ...
            if(!is_wp_error($course_categories) and !empty( $course_categories ) ) {
                foreach( $course_categories as $course_category ) {
                    $course_categories_list[$course_category->name] = $course_category->slug;
                }
            }

			vc_map( array(
				'name'        => esc_html__( '(STM) Courses', 'smarty' ),
				'base'        => 'stm_courses',
				'category'    => esc_html__( 'STM', 'smarty' ),
				'params'      => array(
                    array(
                        'type'       => 'dropdown',
                        'heading'    => esc_html__( 'Sort by Department', 'smarty' ),
                        'param_name' => 'course_category',
                        'value' => $course_categories_list
                    ),
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Courses count', 'smarty' ),
						'param_name' => 'courses_count',
						'description' => esc_html__( 'Courses per page', 'smarty' )
					),
                    array(
                        'type'       => 'checkbox',
                        'heading'    => esc_html__( 'Pagination', 'smarty' ),
...
if( function_exists('stm_add_vc_settings') ) {
	stm_add_vc_settings( 'stm_margin', 'smarty_margin_param', SMARTY_TEMPLATE_URI . '/assets/js/admin/vc/vc_margin.js');
}
...
if( function_exists('stm_add_vc_settings') ) {
	stm_add_vc_settings( 'stm_font', 'smarty_font_param', SMARTY_TEMPLATE_URI . '/assets/js/admin/vc/vc_font.js');
}
...
if( function_exists('stm_add_vc_settings') ) {
	stm_add_vc_settings( 'stm_css', 'smarty_css_param', SMARTY_TEMPLATE_URI . '/assets/js/admin/vc/vc_stm-css.js');
}
