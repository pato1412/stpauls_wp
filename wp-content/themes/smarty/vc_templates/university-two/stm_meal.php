		...
		<ul class="stm-menu stm-menu_antonio stm-menu_tab" role="tablist">
			<?php foreach( $meal_weekdays as $meal_weekday ) : ?>
				<?php if( $meal_weekday->count > 0 ) : ?>
					<li class="stm-menu__item<?php echo (( $tab_order == 1 ) ? ' active' : ''); ?>" role="presentation"><span><a class="stm-menu__item-link" href="#stm-meal-weekdayId-<?php echo esc_attr( $meal_weekday->term_id ); ?>" aria-controls="stm-meal-weekdayId-<?php echo esc_attr( $meal_weekday->term_id ); ?>" role="tab" data-toggle="tab"><?php echo esc_html( $meal_weekday->name ); ?></a></span></li>
					<?php $tab_order = 0; ?>
				<?php endif; ?>
			<?php endforeach; ?>
		</ul>
								...
								<?php if( !empty( $weekdays_items ) ) : ?>
									<div class="col-md-4 col-sm-6 col-xs-12">
										<div class="stm-meal-list">
											<div class="stm-meal-list__heading"><h4><?php echo esc_html( $meal_time_val->name ); ?></h4></div>
					...
					<?php if( $weekdays_data[$meal_weekday->term_id]['showed'] < $weekdays_data[$meal_weekday->term_id]['all'] ): ?>
						<footer class="stm-tab__pane-footer">
							<div class="text-center">
								<a href="#" data-meal-weekday="<?php echo esc_attr( $meal_weekday->term_id ); ?>" data-meal-all="<?php echo esc_attr( $weekdays_data[$meal_weekday->term_id]['all'] ); ?>" data-meal-showed="<?php echo esc_attr( $weekdays_data[$meal_weekday->term_id]['showed'] ); ?>" class="stm-btn stm-btn_outline stm-btn_red stm-btn_md stm-btn_icon-right stm-load-more-deal"><?php esc_attr_e('Load more', 'smarty'); ?><i class="fa fa-angle-right" aria-hidden="true"></i></a>
							</div>
						</footer>
					<?php endif; ?>
				</div>
